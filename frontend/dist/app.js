...
async function loadShippingProviders() {
    try {
        ...
        console.warn('Shipping providers API fetch failed, using defaults:', error.message || error);
    }
    const select = document.getElementById('cShippingProvider');
    if (select) {
        select.innerHTML = `<option value="">Choose courier</option>` + shippingProviders.map(provider => `<option value="${provider.id}">${provider.label}</option>`).join('');
        if (selectedShippingProvider) {
            select.value = selectedShippingProvider;
        }
    }
}
...
function productCardHTML(p) {
    ...
    <div class="product-card card-3d" data-product-id="${p.id}" role="button" tabindex="0">
      ...
      <div class="product-img-wrap">
        <img src="${assetUrl(p.img)}" alt="${escapeAttr(p.name)}" loading="lazy" draggable="false" onerror="this.onerror=null;this.src='/ruh-imperium-logo-old-2026.jpg';this.style.objectFit='contain';this.style.padding='18px';this.parentElement.style.background='#efefef'">
...
function renderCartItems() {
    ...
    meta.innerHTML = currentUser
        ? `Signed in as <strong>${currentUser.name || 'Customer'}</strong>. Coupons and checkout details are ready to go.`
