...
// process.cwd() is backend/ when run inside backend/
...
if (!TOKEN_SECRET) {
  console.warn("[ruh-imperium] AUTH_SECRET is not set — user login/token features are disabled. Set AUTH_SECRET to enable authentication.");
}
...
// ── Data store ───────────────────────────────────────────────────────────────
const DATA_DIR = process.env.DATA_DIR || path.join(ARTIFACT_ROOT, "data");
...
const writeUsers = (u) => writeCollection("users", u);
const readOrders = () => readCollection("orders");
const writeOrders = (o) => writeCollection("orders", o);
const readOtps = () => readCollection("otps");
const writeOtps = (o) => writeCollection("otps", o);
const readSubscribers = () => readCollection("subscribers");
const writeSubscribers = (s) => writeCollection("subscribers", s);
...
function scoreProductForPrompt(product, prompt) {
  ...
  let score = 0;
  const directTerms = text.split(/[^a-z0-9]+/i).filter((t) => t.length > 2);
  directTerms.forEach((term) => { if (blob.includes(term)) score += 2; });
  const toneGroups = [
...
function getLocalScentMatches(prompt, limit = 4) {
  return productCatalog
    .map((p) => ({ product: p, score: scoreProductForPrompt(p, prompt) }))
    .sort((a, b) => b.score - a.score || b.product.stars - a.product.stars || a.product.price - b.product.price)
    .slice(0, limit)
    .map((e) => normalizeCatalogProduct(e.product));
}
...
function currency(amount) { return Number(amount || 0).toLocaleString("en-IN"); }
...
async function sendOtpMessage({ phone, otp }) {
  ...
  const payload = { template_id: MSG91_TEMPLATE_ID, short_url: "0", recipients: [{ mobiles: mobile, otp }] };
...
function buildOrdersCsv(orders) {
  ...
  const lines = orders.map((o) => [o.id,o.createdAt,o.customerName,o.customerEmail,o.customerPhone,o.paymentMethod,o.paymentStatus,o.orderStatus||"pending",o.couponCode||"",o.subtotal,o.discount,o.total,(o.items||[]).map((i)=>`${i.name} (${i.size}) x ${i.qty}`).join(" | ")].map(csvEscape).join(","));
...
router.post("/ai-scent-chat", async (req, res) => {
  try {
    ...
    const catalogLines = productCatalog.map((p) => `${p.id}. ${p.name} | ${p.cat} | notes: ${p.notes} | tags: ${(p.tags||[]).join(", ")} | price: ₹${p.price}`).join("\n");
    try {
      ...
      const parts = [];
      (data?.output || []).forEach((item) => (item.content || []).forEach((c) => { if (typeof c.text === "string") parts.push(c.text); }));
...
router.post("/newsletter/subscribe", async (req, res) => {
  try {
    ...
    if (subscribers.find((s) => s.email === email)) { res.json({ subscribed: true, alreadySubscribed: true, email }); return; }
...
router.post("/auth/signup", async (req, res) => {
  try {
    ...
    const users = await readUsers();
    if (users.some((u) => u.email === em)) { res.status(409).json({ error: "An account with this email already exists." }); return; }
...
router.post("/auth/request-otp", async (req, res) => {
  try {
    ...
    const users = await readUsers();
    let targetUser = users.find((u) => (email && u.email === email) || (phone && u.phone === phone));
    if (!targetUser) {
      if (createIfMissing) {
    ...
    const otps = (await readOtps()).filter((o) => o.userId !== targetUser.id && o.expiresAt > Date.now());
    ...
    res.json({ message: delivery.mode === "sms" ? `OTP sent to ${targetUser.phone || phone}.` : `OTP generated. SMS is not configured yet.`, identifier: email || phone, delivery: delivery.mode, previewOtp: delivery.previewOtp || "" });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.post("/auth/verify-otp", async (req, res) => {
  try {
    ...
    const match = otps.find((o) => o.userId === user.id && o.purpose === "login" && o.expiresAt > Date.now());
    ...
    await writeOtps(otps.filter((o) => o.userId !== user.id));
...
router.post("/orders/request-otp", async (req, res) => {
  try {
    ...
    if (!email && !phone) { res.status(400).json({ error: "Email or phone is required." }); return; }
    const code = generateOtp();
    const otps = (await readOtps()).filter((o) => !(o.userId === authUser.id && o.purpose === "order") && o.expiresAt > Date.now());
    ...
    res.json({ message: delivery.mode === "sms" ? `Order OTP sent to ${phone || email}.` : `Order OTP generated. SMS is not configured yet.`, identifier: phone || email, delivery: delivery.mode, previewOtp: delivery.previewOtp || "" });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.post("/orders/verify-otp", async (req, res) => {
  try {
    ...
    if ((!email && !phone) || !otp) { res.status(400).json({ error: "Email or phone and OTP are required." }); return; }
    const otps = await readOtps();
    const match = otps.find((o) => o.userId === authUser.id && o.purpose === "order" && o.expiresAt > Date.now() && ((email && o.email === email) || (phone && o.phone === phone)));
    ...
    await writeOtps(otps.filter((o) => !(o.userId === authUser.id && o.purpose === "order")));
...
router.post("/payments/razorpay/verify", async (req, res) => {
  try {
    ...
    const orders = await readOrders();
    const order = orders.find((o) => o.razorpayOrderId === req.body.orderId && o.userId === authUser.id);
    if (order) {
      ...
      await writeOrders(orders);
      try { await sendOrderNotification(order, "paid", req); } catch (e) { logger.error({ err: e }, "Payment email failed"); }
    }
...
router.post("/orders/cod", async (req, res) => {
  try {
    ...
    orders.push(order);
    await writeOrders(orders);
    try { await sendOrderNotification(order, "placed", req); } catch (e) { logger.error({ err: e }, "Order email failed"); }
...
router.get("/orders", async (req, res) => {
  try {
    ...
    const orders = (await readOrders()).filter((o) => o.userId === authUser.id).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    res.json({ orders });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.get("/admin/orders", async (req, res) => {
  try {
    ...
    const orders = (await readOrders()).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    res.json({ orders });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.get("/admin/subscribers", async (req, res) => {
  try {
    const adminUser = await getAdminUser(req);
    if (!adminUser) { res.status(403).json({ error: "Admin access required." }); return; }
    const subscribers = (await readSubscribers()).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
...
router.delete("/admin/subscribers/:subscriberId", async (req, res) => {
  try {
    ...
    if (!adminUser) { res.status(403).json({ error: "Admin access required." }); return; }
    const subscribers = await readSubscribers();
    const next = subscribers.filter((s) => s.id !== req.params.subscriberId);
...
router.patch("/admin/orders/:orderId/status", async (req, res) => {
  try {
    ...
    } catch (e) { logger.error({ err: e }, "Order status email failed"); }
    res.json({ order });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
